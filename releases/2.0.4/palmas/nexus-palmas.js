(() => {
    ...
    const CONFIG = {
        ...
        mainDataPath: 'Ciclo-semanas-setores/palmas',
        ecosystemKey: 'palmas',
        ecosystemName: 'PALMAS',
        ...
        pendingStorageKey: 'ciclosNotasPerdidasPendingCapture_palmas_V1',
        panelId: 'ciclos-tm-panel-palmas',
        requestPollMs: 10000,
    ...
    try {
        if (typeof GM_registerMenuCommand === 'function') {
            GM_registerMenuCommand('Abrir painel CICLOS PALMAS', () => {
                if (!isTargetPage()) {
        ...
        console.debug('[CICLOS/PALMAS] Menu não disponível:', error);
    }

    waitForTargetPage(120000)
        .then(found => {
            if (!found) {
                console.info('[CICLOS/PALMAS] Página 297 não detectada. O script permaneceu inativo nesta aba.');
                return;
            }
            return startIntegration(false);
        })
        .catch(error => {
            console.error('[CICLOS/PALMAS] Falha na inicialização:', error);
            if (isTargetPage()) {
                ensurePanel();
    ...
    async function init() {
        ...
        pollingTimer = window.setInterval(() => {
            if (!isBusy) refreshContext(true).catch(error => {
                console.error('[CICLOS/PALMAS] Erro ao consultar solicitações:', error);
    ...
    function ensurePanel() {
        ...
        style.textContent = `
            ...
            #${CONFIG.panelId} .ciclos-head{display:flex;align-items:center;justify-content:space-between;padding:13px 15px;background:linear-gradient(135deg,#6d28d9,#7c3aed);color:#fff}
        ...
        panel.innerHTML = `
            <div class="ciclos-head">
                <div>
                    <div class="ciclos-title">PALMAS • Atualização <span style="font-size:10px;opacity:.8">v1.9</span></div>
                    <div class="ciclos-sub">Notas Perdidas → Nexus PALMAS</div>
                </div>
                <button type="button" class="ciclos-close" title="Minimizar">−</button>
            </div>
            <div class="ciclos-body">
                <label for="ciclos-tm-profile">Distribuidor • PALMAS</label>
                ...
                <div id="ciclos-tm-match" class="ciclos-match">Selecione um distribuidor de PALMAS.</div>
                <div id="ciclos-tm-status" class="ciclos-status">Carregando distribuidores de PALMAS...</div>
            </div>
        `;
    ...
    async function refreshContext(silent = false) {
        ...
        if (newestPending) {
            ...
            setStatus(`Solicitação de PALMAS recebida para “${newestPending.profile}”. Clique para atender.`, 'warning');
            ...
            if (!silent) setStatus(`PALMAS: selecione o distribuidor e clique para filtrar e enviar.`, 'info');
            actionButton.textContent = 'Filtrar e enviar dados';
        } else {
            if (!silent) {
                const qtd = availableProfiles.length;
                setStatus(`${qtd} distribuidor${qtd === 1 ? '' : 'es'} carregado${qtd === 1 ? '' : 's'} exclusivamente de PALMAS.`, qtd ? 'success' : 'warning');
            }
            actionButton.textContent = 'Filtrar e enviar dados';
        }
    ...
    function previewMatch() {
        ...
        if (!profile) {
            matchBox.textContent = 'Selecione um distribuidor de PALMAS.';
            return;
        }
    ...
    async function startFilterAndSend() {
        ...
        const profile = profileSelect.value;
        if (!profile) {
            setStatus('Selecione um distribuidor de PALMAS antes de continuar.', 'warning');
            return;
        }
        ...
        try {
            ...
            setStatus(`PALMAS → “${match.text}”. A página será filtrada agora.`, 'success');
        ...
        try {
            ...
            if (countMismatches.length) {
                const detail = countMismatches.slice(0, 5)
                    .map(item => `${item.nomeOriginal}: capturadas ${item.total}, Contagem ${item.contagemPagina}`)
                    .join(' | ');
            ...
            console.error('[CICLOS/PALMAS] Erro na captura:', error);
            GM_deleteValue(CONFIG.pendingStorageKey);
    ...
    async function collectAllReportRows() {
        ...
        for (let page = 1; page <= CONFIG.maxPaginationPages; page++) {
            ...
            Object.entries(current.regionFooterCounts || {}).forEach(([key, item]) => {
                ...
                // Se o APEX repetir o rodapé durante a paginação, mantém a maior contagem.
            ...
            if (!changed) {
                console.warn('[CICLOS/PALMAS] A paginação não alterou o relatório; a captura foi encerrada na página atual.');
                break;
            }
        }

        return { rows: allRows, headers: masterHeaders, regionFooterCounts };
    }
    ...
    function extractCurrentPage(region) {
        ...
        if (!table) return { headers: [], rows: [], regionFooterCounts: {} };
        ...
        tableRows.forEach(tr => {
            ...
            // O APEX adiciona uma linha de totalização após cada Região. Ela não é uma nota,
            // porém a célula "Contagem" é a fonte oficial para validar se capturamos TODAS
            // as notas daquela Região.
            if (dataCells.some(cell => cell.classList.contains('a-IRR-aggregate'))) {
                const countCell = dataCells.find(cell => {
                    const typeText = normalizeText(cleanCellText(cell.querySelector('.a-IRR-aggregate-type') || cell));
                    return typeText.includes('contagem');
                });
                if (controlRegion && countCell) {
                    const valueText = cleanCellText(countCell.querySelector('.a-IRR-aggregate-value') || countCell);
                    const count = parseInteger(valueText);
                    if (Number.isFinite(count) && count >= 0) {
                        const regionKey = normalizeKey(controlRegion) || 'sem_regiao';
                        regionFooterCounts[regionKey] = { nomeOriginal: controlRegion, count };
                    }
                }
                return;
            }
        ...
        return { headers, rows, regionFooterCounts };
    }
    ...
    function buildPayload(rows, fallbackHeaders, pendingCapture, regionFooterCounts = {}) {
        ...
        if (!rows.length) {
            return {
                ...
                lateCarryRule: 'region-qtd-remarcada-v2-file-rule',
                regionCountRule: 'apex-footer-count-v1',
        ...
        Object.entries(regions).forEach(([regionKey, region]) => {
            ...
            const matches = pageCount === null ? true : total === pageCount;
    ...
    function setApexItem(id, value) {
        ...
        try {
            ...
            console.debug('[CICLOS/PALMAS] apex.item.setValue indisponível:', error);
        }
